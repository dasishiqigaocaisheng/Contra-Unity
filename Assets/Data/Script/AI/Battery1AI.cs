...
namespace Contra.AI
{
    public class Battery1AI : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            fsm.AddState("Wait",
                ...
                update: (x) =>
                {
                    if (Mathf.Abs(Player.ClosestAliveOne(transform.position).transform.position.x - transform.position.x) < 10)
                        x.GotoState("Active");
                },
                exit: (x) =>
                {
            ...
            fsm.AddState("Active",
                ...
                update: (x) =>
                {
                    ...
                    else
                    {
                        ...
                        if (dir_code_rec == dir_code_exp)
                            x.GotoState("Fire", (Vector3)_DirDecode[dir_code_rec.Round()]);
                        else
                            x.GotoState("Active");
                    }
                });
            fsm.AddState("Fire",
               enter: (x) =>
               {
                   ...
                   Vector3 dir = x.GetAttachedData<Vector3>();
                   Bullet.Spawn("Enemy", transform.position + dir * 1.5f, dir);
                   DOVirtual.DelayedCall(0.2f, () =>
                   {
                   ...
                   }).SetId(this).onComplete = () =>
                       {
                           DOVirtual.DelayedCall(0.2f, () =>
                           {
                               Vector3 dir = x.GetAttachedData<Vector3>();
                               Bullet.Spawn("Enemy", transform.position + dir * 1.5f, dir);
                               x.Attached_FSM.Controller.CancelBlock();
                           }).SetId(this);
                       };
            ...
            fsm.SetBeginPath("Wait", (x) => GameManager.Inst.Started);
            ...
            fsm.AddTransferPath("Active", "Fire", FSM.MANUAL);
            fsm.AddTransferPath("Fire", "Active", FSM.MANUAL);
